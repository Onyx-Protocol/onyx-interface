...
import { GetOTokenApySimulationsOutput, GetOTokenInterestRatesInput } from './types';

export * from './types';

const REFERENCE_AMOUNT_WEI = 1e4;

const getOTokenApySimulations = async ({
  multicall,
  reserveFactorMantissa,
  interestRateModelContractAddress,
}: GetOTokenInterestRatesInput): Promise<GetOTokenApySimulationsOutput> => {
  ...
  const contractCallContext: ContractCallContext = {
    reference: 'getOTokenRates',
    contractAddress: interestRateModelContractAddress,
    abi: interestModelAbi,
    calls,
  };

  const oTokenBalanceCallResults: ContractCallResults = await multicall.call(contractCallContext);
  const apySimulations = formatToApySnapshots({ oTokenBalanceCallResults });

  return { apySimulations };
};

export default getOTokenApySimulations;
