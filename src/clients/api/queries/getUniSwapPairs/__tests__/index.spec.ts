...
import getUniSwapPairs from '..';

describe('api/queries/getUniSwapPairs', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      await getUniSwapPairs({
        multicall,
        tokenCombinations: [],
      });

      throw new Error('getUniSwapPairs should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns pairs in the right format on success', async () => {
    const multicall = {
      call: jest.fn(async () => fakeMulticallResponses.uniSwapRouter.getReserves),
    } as unknown as Multicall;

    const res = await getUniSwapPairs({
      multicall,
