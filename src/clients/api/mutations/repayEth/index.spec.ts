...
import repayEth, { REPAYMENT_ETH_BUFFER_PERCENTAGE } from '.';
...
describe('api/mutation/repayEth', () => {
  describe('repay partial ETH loan', () => {
    beforeEach(() => {
      (getOTokenContract as jest.Mock).mockImplementationOnce(() => ({
        methods: {
          repayBorrow: () => ({
    ...
    test('throws an error when request fails', async () => {
      ...
      try {
        ...
        throw new Error('repayEth should have thrown an error but did not');
      } catch (error) {
  ...
  describe('repay full ETH loan', () => {
    ...
    test('returns transaction receipt when request succeeds', async () => {
      ...
      expect(repayBehalfExplicit).toHaveBeenCalledWith(fakeAddress, OETH_TOKENS.eth.address);
      expect(sendMock).toHaveBeenCalledTimes(1);
      expect(sendMock).toHaveBeenCalledWith({
        from: fakeAddress,
        value: fakeAmountWei.multipliedBy(1 + REPAYMENT_ETH_BUFFER_PERCENTAGE).toFixed(0),
      });
    });
  });
});
