...
describe('api/mutation/withdrawFromFarm', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      ...
      throw new Error('withdrawFromFarm should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns receipt when request succeeds', async () => {
    ...
    const response = await withdrawFromFarm({
      masterChefContract: fakeContract,
      fromAccountAddress: fakeFromAccountsAddress,
      pid: fakePid,
      amountWei: fakeAmountWei,
