...
import { getAllowance, repayNonEthOToken, useGetUserMarketInfo } from 'clients/api';
...
describe('pages/Dashboard/BorrowRepayModal/Repay', () => {
  ...
  it('renders without crashing', () => {
    renderComponent(<Repay asset={fakeAsset} onClose={noop} isXcnEnabled />);
  });

  it('displays correct token borrow balance', async () => {
    const { getByText } = renderComponent(<Repay asset={fakeAsset} onClose={noop} isXcnEnabled />, {
      authContextValue: {
        account: {
  ...
  it('disables submit button if an amount entered in input is higher than token borrow balance', async () => {
    ...
    const { getByText, getByTestId } = renderComponent(
      <Repay asset={customFakeAsset} onClose={noop} isXcnEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('disables submit button if an amount entered in input is higher than token wallet balance', async () => {
    const { getByText, getByTestId } = renderComponent(
      <Repay asset={fakeAsset} onClose={noop} isXcnEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('lets user repay borrowed tokens, then displays successful transaction modal and calls onClose callback on success', async () => {
    ...
    (repayNonEthOToken as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);

    const { getByText, getByTestId } = renderComponent(
      <Repay asset={fakeAsset} onClose={onCloseMock} isXcnEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
    ...
    await waitFor(() => expect(repayNonEthOToken).toHaveBeenCalledTimes(1));
    expect(repayNonEthOToken).toHaveBeenCalledWith({
  ...
  it('lets user repay full loan that is not in ETH', async () => {
    (repayNonEthOToken as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);

    const { getByText } = renderComponent(
      <Repay asset={fakeAsset} onClose={jest.fn()} isXcnEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('lets user repay full loan that is in ETH', async () => {
    (repayNonEthOToken as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);

    const fakeEthAsset: Asset = {
      ...fakeAsset,
      token: TOKENS.eth,
    };

    const { getByText } = renderComponent(
      <Repay asset={fakeEthAsset} onClose={jest.fn()} isXcnEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
    ...
    expect(repayNonEthOToken).toHaveBeenCalledWith({
      amountWei: fakeEthAsset.borrowBalance.multipliedBy(1e18), // Convert borrow balance to wei
      fromAccountAddress: fakeAccountAddress,
      isRepayingFullLoan: true,
    });
  });
