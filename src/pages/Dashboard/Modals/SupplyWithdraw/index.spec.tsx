...
const fakeGetOTokenBalance = new BigNumber('111');
...
describe('pages/Dashboard/SupplyWithdrawUi', () => {
  ...
  it('renders without crashing', async () => {
    renderComponent(() => (
      <SupplyWithdraw onClose={jest.fn()} asset={asset} isXcnEnabled assets={assetData} />
    ));
  });
  ...
  describe('Supply form', () => {
    ...
    it('lets user supply ETH, then displays successful transaction modal and calls onClose callback on success', async () => {
      const customFakeAsset: Asset = {
        ...fakeAsset,
        token: TOKENS.eth,
      ...
      (supplyEth as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);
      ...
      await waitFor(() => expect(supplyEth).toHaveBeenCalledWith({ amountWei: expectedAmountWei }));
    ...
    it('lets user supply non-ETH tokens, then displays successful transaction modal and calls onClose callback on success', async () => {
      const customFakeAsset: Asset = {
        ...fakeAsset,
        token: TOKENS.usdt,
        walletBalance: new BigNumber('11'),
      ...
      (supplyNonEth as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);
      ...
      await waitFor(() =>
        expect(supplyNonEth).toHaveBeenCalledWith({ amountWei: expectedAmountWei }),
      );
      expect(onCloseMock).toHaveBeenCalledTimes(1);
      await waitFor(() =>
        expect(openSuccessfulTransactionModal).toHaveBeenCalledWith({
  ...
  describe('Withdraw form', () => {
    ...
    it('redeem is called when full amount is withdrawn', async () => {
      (getOTokenBalanceOf as jest.Mock).mockImplementationOnce(async () => ({
        balanceWei: fakeGetOTokenBalance,
      }));
      const { getByText } = renderComponent(
        () => <SupplyWithdraw onClose={jest.fn()} asset={asset} isXcnEnabled assets={assetData} />,
        {
      ...
      fireEvent.click(submitButton);
      await waitFor(() => expect(redeem).toHaveBeenCalledWith({ amountWei: fakeGetOTokenBalance }));
    });
