...
export const SupplyWithdrawUi: React.FC<SupplyWithdrawUiProps & SupplyWithdrawProps> = ({
  ...
  const tokenInfo: LabeledInlineContentProps[] = asset
    ? [
        ...
        },
        {
          ...
          iconSrc: TOKENS.xcn,
          children: formatToReadablePercentage(asset.xcnSupplyApy),
        },
      ]
    : [];
  ...
  const renderTabContent = ({
    ...
    const maxInput = React.useMemo(() => {
      ...
      if (type === 'supply' && asset.supplyCaps.isGreaterThan(0)) {
        const decimal = new BigNumber(10).pow(asset.token.decimals);
        const totalSupplies = asset.cash
          .plus(asset.treasuryTotalBorrows.times(decimal))
          .minus(asset.totalReserves);
...
const SupplyWithdrawModal: React.FC<SupplyWithdrawUiProps> = props => {
  const { asset, assets, isXcnEnabled, onClose, ...rest } = props;
  const { account: { address: accountAddress = '' } = {} } = useContext(AuthContext);

  const isNFTMarket = (asset || {})?.token?.decimals === 0;
  ...
  const { data: oTokenBalanceData } = useGetOTokenBalanceOf(
    { accountAddress, oTokenId: asset.token.id },
  ...
  const onSubmitWithdraw: AmountFormProps['onSubmit'] = async value => {
    ...
    if (amountEqualsSupplyBalance && oTokenBalanceData?.balanceWei) {
      const res = await redeem({ amountWei: new BigNumber(oTokenBalanceData.balanceWei) });
