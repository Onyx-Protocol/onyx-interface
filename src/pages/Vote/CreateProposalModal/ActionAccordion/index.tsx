...
const ActionAccordion: React.FC = () => {
  ...
  const [{ value: actions }, { error: errors }, { setValue }] =
    useField<{ target: string; value: string; signature: string; callData: string[] }[]>('actions');
            ...
            {actions.map((action, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <FormikTextField
                    name={`actions.${idx}.value`}
                    data-testid={`actions.${idx}.value`}
                    placeholder={t('vote.createProposalForm.value')}
                    maxLength={42}
                    css={[styles.formTopMargin, styles.formBottomMargin]}
                    displayableErrorCodes={[ErrorCode.ACTION_VALUE_NOT_VALID]}
                    label={t('vote.createProposalForm.value')}
                  />
