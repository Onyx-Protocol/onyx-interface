...
export const parameters = {
  ...
  backgrounds: {
    ...
    values: [
      {
        name: 'Primary',
        value: PALETTE[0].background.default,
      },
      {
        name: 'Paper',
        value: PALETTE[0].background.paper,
      },
      {
        name: 'White',
  ...
  msw: {
    handlers: {
      testnet: [
        ...
        rest.get('https://testnetapi.onyx.org/api/xcn', (req, res, ctx) => {
          return res(ctx.json(GovernanceResponse));
        }),
        rest.get('https://testnetapi.onyx.org/api/proposals', (req, res, ctx) => {
          return res(ctx.json(VotersReponse));
        }),
        rest.get('https://testnetapi.onyx.org/api/transactions?page=1', (req, res, ctx) => {
          return res(ctx.json(TransactionResponse));
        }),
        rest.get(
          'https://testnetapi.onyx.org/api/market_history/graph?asset=0x74469281310195A04840Daf6EdF576F559a3dE80&type=1hr&limit=168',
          (req, res, ctx) => {
            return res(
              ctx.json({
                data: { result: [] },
